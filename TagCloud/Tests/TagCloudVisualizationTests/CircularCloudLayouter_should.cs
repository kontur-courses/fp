...
namespace TagCloud.TagCloudVisualisation.Tests
{
    ...
    class CircularCloudLayouter_Should
    {
        ...
        public void PutNextRectangle_ShouldReturnRectangle_WithGivenSize(int width, int height)
        {
            ...
            rectangle.Value.Size.Should().Be(new Size(width, height));
        }
        ...
        public void PutNextRectangle_ShouldBePutFirstRectangle_InCenter(int centerX, int centerY)
        {
            ...
            firstRectangle.Value.Location.Should().Be(center);
        }
        ...
        {
            var firstRectangle = cloudLayouter.PutNextRectangle(new Size(rect1Width, rect1Height)).Value;
            var secondRectangle = cloudLayouter.PutNextRectangle(new Size(rect2Width, rect2Height)).Value;
        ...
        public void PutNextRectangle_AllRectanglesOnOneCloud_Should_NotIntersect(int countRectangles)
        {
            ...
            for (int stage = 0; stage < 50; stage++) // каждое countRectangle проверяется в 50 проходов
            {
                for (int i = 0; i < countRectangles; i++)
                {
                    ...
                    rectangles.Add(cloudLayouter.PutNextRectangle(size).Value);
                }
