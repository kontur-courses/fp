...
namespace TagCloud
{
    ...
    public static class Result
    {
        ...
        public static Result<T> FailIfNull<T>(Func<T> f, string e = null)
        {
            var value = f();
            if (value == null) return Fail<T>(e);
            return Ok(value);
        }
        
        public static Result<T> FailIfFalse<T>(this Result<T> result, Predicate<T> f, string e = null)
        {
            if (!result.IsSuccess) return result;
            if (!f(result.Value)) return Fail<T>(e);
