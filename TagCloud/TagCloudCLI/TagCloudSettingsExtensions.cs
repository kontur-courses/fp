...
{
    internal static class TagCloudSettingsExtensions
    {
        ...
        {
            return Validate(settings, settings => settings.backgroundColor.IsKnownColor,
                $"Unknown color {settings.backgroundColor.Name}");
        }
        ...
        private static Result<TagCloudSettings> ValidateFontFamily(TagCloudSettings settings)
        {
            return Validate(settings,
                tagCloudSettings =>
                    FontFamily.Families.Any(fontFamily => fontFamily.Name == tagCloudSettings.fontFamily.Name),
