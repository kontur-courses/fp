    ...
    public class Result_Should
    {
        ...
        public void RefineError_AddErrorMessageBeforePreviousErrorText()
        {
            ...
            calculation.RefineError("Posting results to db")
                       .Should()
                       .BeEquivalentTo(Fail<None>("Posting results to db. No connection"));
        }
        ...
        public void ReplaceError_IfFail()
        {
            Fail<None>("error")
                .ReplaceError(e => "replaced")
                .Should()
                .BeEquivalentTo(Fail<None>("replaced"));
        }

        [Test]
        public void ReturnsFail_FromResultOf_OnException()
        {
            var res = Of<int>(() => { throw new Exception("123"); });

            res.Should()
               .BeEquivalentTo(Fail<int>("123"));
        }
        ...
        public void RunThen_WhenOk()
        {
            var res = Ok(42)
                .Then(n => n + 10);
            res.Should()
               .BeEquivalentTo(Ok(52));
        }

        [Test]
        public void RunThen_WhenOk_ComplexScenario()
        {
            var parsed = Ok("1358571172")
                .Then(int.Parse);
            var res = parsed.Then(i => Convert.ToString(i, 16))
                            .Then(hex => parsed.GetValueOrThrow() + " -> " + Guid.Parse(hex + hex + hex + hex));
            res.Should()
               .BeEquivalentTo(Ok("1358571172 -> 50fa26a4-50fa-26a4-50fa-26a450fa26a4"));
        }

        [Test]
        public void RunThen_WhenOk_Scenario()
        {
            var res = Ok("1358571172")
                      .Then(int.Parse)
                      .Then(i => Convert.ToString(i, 16))
                      .Then(hex => Guid.Parse(hex + hex + hex + hex));
            res.Should()
               .BeEquivalentTo(Ok(Guid.Parse("50FA26A450FA26A450FA26A450FA26A4")));
        }
