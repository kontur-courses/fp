...
public class CircularCloudLayouterTests
{
    ...
    public void PutNextRectangle_PutFirstRectangleOnCenter()
    {
        ...
        rectangle.GetValueOrThrow().Location.Should().BeEquivalentTo(expectedLocation);
    }

    [Test]
    public void PutNexRectangle_ReturnFail_WhenRectangleSizeLessThanZero()
    {
        var invalidSize = new Size(-1, -1);

        var result = sut.PutNextRectangle(invalidSize);

        result.IsSuccess.Should().BeFalse();
        result.Error.Should().Be("Rectangle sizes must be greater than zero");
    }
