...
{
    public class TagsCloudOptionsValidator
    {
        ...
        public static Result<IImageOptions> ValidateImageOptions(CommandLineOptions options)
        {
            ...
            return width > 0 && height > 0
                ? Result.Ok((IImageOptions) new ImageOptions(width, height, options.OutputDirectory, options.OutputFileName, options.OutputFileExtension))
                : Result.Fail<IImageOptions>("Incorrect image size.");
        }

        public static Result<FontOptions> ValidateFontOptions(CommandLineOptions options)
        {
            ...
            return color.IsSuccess && family.IsSuccess
                ? Result.Ok(new FontOptions(family.Value, color.Value))
                : Result.Fail<FontOptions>(!color.IsSuccess ? color.Error : family.Error);
        }
