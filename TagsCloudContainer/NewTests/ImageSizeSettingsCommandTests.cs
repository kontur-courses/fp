...
namespace TagsCloudContainer.NewTests
{
    class ImageSizeSettingsCommandTests
    {
        ...
        [Test]
        public void TryChangeSettings_ShouldChangeSettingsToDefault_IfNoArguments()
        {
            var arguments = new[] { "size" };
            settings.ImageSize = new Size(10, 10);

            var result = command.TryChangeSettings(arguments, settings);
            result.Value.ImageSize.Should().Be(Size.Empty);
        }

        [Test]
        public void TryChangeSettings_ShouldChangeSettingsToDefault_IfArgumentAuto()
        {
            var arguments = new[] { "size", "auto" };
            settings.ImageSize = new Size(10, 10);
