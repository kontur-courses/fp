...
namespace TagsCloudContainer.Functional
{
    ...
    public static class Result
    {
        ...
        public static Result<IEnumerable<TOutput>> Map<TInput, TOutput>(
            this Result<IEnumerable<TInput>> input,
            Func<TInput, Result<TOutput>> continuation)
        {
            return input.Then(enumerable =>
            {
                var outputs = new List<TOutput>();
                foreach (var element in enumerable)
                {
                    var result = continuation(element);
                    if (!result.IsSuccess)
                        return Fail<IEnumerable<TOutput>>(result.Error);
                    outputs.Add(result.Value);
                }

                return Ok((IEnumerable<TOutput>) outputs);
            });
        }
