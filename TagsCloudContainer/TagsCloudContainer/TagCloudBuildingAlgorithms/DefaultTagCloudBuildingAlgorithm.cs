...
{
    public class DefaultTagCloudBuildingAlgorithm : ITagCloudBuildingAlgorithm
    {
        ...
        public Result<IEnumerable<Tag>> GetTags(Dictionary<string, int> wordsFrequency)
        {
            var result = new List<Tag>();
            if (wordsFrequency.Count == 0)
                return Result.Ok((IEnumerable<Tag>)result);
