...
namespace TagsCloud.Tests
{
    ...
    public class WordAnalyzerTests
    {
        ...
        {
            var analyzer = new WordAnalyzer(new List<Result<string>> { "стол", "стол", "бегать", "бегать", "бегать"});
            var frequency = analyzer.GetWordFrequency().Value;
        ...
        {
            var analyzer = new WordAnalyzer(new List<Result<string>> { "стол", "стол", "и", "но", "что", "я", "мы" });
            var frequency = analyzer.GetWordFrequency().Value;
        ...
        {
            var analyzer = new WordAnalyzer(new List<Result<string>> { "стол", "бегать", "и", "я", "я"});
            var frequency = analyzer.GetWordFrequency(new HashSet<PartOfSpeech>{PartOfSpeech.Noun, PartOfSpeech.Verb}).Value;
            frequency.Should().HaveCount(2)
        ...
        {
            var analyzer = new WordAnalyzer(new List<Result<string>>
        ...
        {
            var analyzer = new WordAnalyzer(new List<Result<string>> { "стол", "стол", "стул", "стул",
                "стул", "бегать", "бегать", "я", "и"});
            var frequency = analyzer.GetSpecificWordFrequency(new List<PartOfSpeech> {PartOfSpeech.Noun}).Value;
            frequency.Should().HaveCount(2)
        ...
        public void GetSpecificWordFrequency_CountCorrectlyFrequency_OnMultiplePartOfSpeech()
        {
            var analyzer = new WordAnalyzer(new List<Result<string>> { "стол", "стол", "стул", "стул",
                "стул", "бегать","бегать", "я", "и"});
            var frequency = analyzer.GetSpecificWordFrequency(new List<PartOfSpeech> { PartOfSpeech.Noun, PartOfSpeech.Verb }).Value;
        ...
        public void GetSpecificWordFrequency_ReturnsInfinitive_OnAppropriateParameter()
        {
            ...
            var frequency = analyzer.GetSpecificWordFrequency(new List<PartOfSpeech>
                {PartOfSpeech.Noun, PartOfSpeech.Verb}).Value;
            frequency.Should().HaveCount(2)
                .And.Contain(new KeyValuePair<string, int>("стол", 3))
                .And.Contain(new KeyValuePair<string, int>("бегать", 3));
        }
