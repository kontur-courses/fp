...
namespace TagsCloudResult
{
    ...
    {
        public static Result<IEnumerable<LayoutWord>> Select(IEnumerable<string> wordsList, AppSettings settings)
        {
            var wordSetting = settings.WordsSetting;
            ...
            if (wordSetting.Color != "random" && !Color.FromName(wordSetting.Color).IsKnownColor)
        ...
        {
            return new Size(word.Length * ((int) Math.Floor(font.Size) - frequency / 2),
                font.Height);
        }
