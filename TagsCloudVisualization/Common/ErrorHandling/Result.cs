...
namespace TagsCloudVisualization.Common.ErrorHandling
{
    public class None
    {
        private None()
    ...
    public struct Result<T>
    {
        ...
        public T GetValueOrThrow()
        {
            ...
            throw new InvalidOperationException(
                $"При попытке извлечения значения типа {typeof(T)} возникла ошибка: {Error}");
        }
    ...
    public static class Result
    {
        ...
        public static Result<TInput> OnSuccess<TInput>(
            this Result<TInput> input,
            Action<TInput> handleSuccess)
        {
            if (input.IsSuccess) handleSuccess(input.Value);
            return input;
        }
        ...
        public static Result<TInput> And<TInput>(
            this Result<TInput> input,
            Action<TInput> continuation,
            string err)
        {
