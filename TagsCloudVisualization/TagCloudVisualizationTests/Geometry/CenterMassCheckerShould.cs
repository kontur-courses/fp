...
namespace TagCloudVisualizationTests.Geometry
{
    ...
    class CenterMassCheckerShould
    {
        ...
        {
            var rectangles = new List<Rectangle>() { circularCloudLayouter.PutNextRectangle(new Size(2, 2)).Value };
        ...
        {
            var rectangles = Enumerable.Range(0, 9).Select(b => circularCloudLayouter.PutNextRectangle(new Size(2, 2)).Value).ToList();
            CenterMassChecker.FindCenterMass(rectangles).Should().Be(new PointF(0, 0));
        } 
    }
}
