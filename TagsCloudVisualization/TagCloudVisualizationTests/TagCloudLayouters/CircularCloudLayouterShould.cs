...
namespace TagCloudVisualizationTests.TagCloudLayouters
{
    ...
    class CircularCloudLayouterShould
    {
        ...
        public void LocateTopLeftCornerOfFirstRectangleInCenter()
        {
            ...
            circularCloudLayouter.PutNextRectangle(size).Value.Location.Should().Be(new Point(299, 298));
        }
        ...
        public void ReturnPositionClosestToCenter()
        {
            sizesForTesting
                .Take(4)
                .Select(size => circularCloudLayouter.PutNextRectangle(size).Value.Location)
                .Should()
        ...
        {
            rectangles = sizesForTesting.Select(size => circularCloudLayouter.PutNextRectangle(size).Value).ToList();
        ...
        public void CreateCloud_WithCircleShape(int rectanglesCount)
        {
            ...
            for (var i = 0; i < rectanglesCount; i++)
                rectangles.Add(circularCloudLayouter.PutNextRectangle(new Size(random.Next(1, 50), random.Next(1, 50))).Value);
            var corners = GetAllPoints();
            var radiuses = GetFourRadiuses(corners).ToList();
        ...
        {
            var random = new Random();
            rectangles = Enumerable.Range(0, count).Select(b => circularCloudLayouter.PutNextRectangle(new Size(random.Next(min, max), random.Next(min, max))).Value).ToList();
