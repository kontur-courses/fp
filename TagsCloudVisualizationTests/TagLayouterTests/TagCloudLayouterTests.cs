...
public class TagCloudLayouterTests
{
    ...
    private ImageSettings imageSettings = new ImageSettings(200, 200);
    ...
    public void GetTags_OneTag_ShouldReturnCorrectTag()
    {
        ...
        var result = layouter.GetTags().GetValueOrThrow();
    ...
    public void GetTags_TwoTags_ShouldReturnCorrectTags()
    {
        ...
        var result = layouter.GetTags().GetValueOrThrow().OrderByDescending(t => t.Size).ToList();
        var excepted = new List<Tag> { new Tag("a", 20, new Rectangle(new Point(0, 0), new Size(25, 34)), new FontFamily(settings.FontFamily)),
                                       new Tag("b", 20, new Rectangle(new Point(0, 0), new Size(25, 34)), new FontFamily(settings.FontFamily))};
    ...
    public void GetTags_ThreeTagsWithDifferentCount_ShouldReturnCorrectTags()
    {
        ...
        var result = layouter.GetTags().GetValueOrThrow().OrderByDescending(t => t.Size).ToList();
        var excepted = new List<Tag> { new Tag("a", 20, new Rectangle(new Point(0, 0), new Size(25, 34)), new FontFamily(settings.FontFamily)),
                                       new Tag("b", 15, new Rectangle(new Point(0, 0), new Size(19, 25)), new FontFamily(settings.FontFamily)),
                                       new Tag("c", 10, new Rectangle(new Point(0, 0), new Size(12, 17)), new FontFamily(settings.FontFamily)) };
        for (var i = 0; i < excepted.Count; i++)
        {
            result[i].Size.Should().Be(excepted[i].Size);
            result[i].Rectangle.Size.Should().Be(excepted[i].Rectangle.Size);
